...
router.get('/category/:category', (req, res) => {
    ...
    dbConnection.query(sql, (err, queryResult) => {
        if (err) {
            ...
            if (id_cat) {
                dbConnection.query(`select * from post where id_category =${id_cat}`, (err, result) => {
                    if (err) {
                        console.log(err);
                    }
                    else {
...
router.get('/get-post/:id', (req, res) => {
    ...
    dbConnection.query(sql, (err, queryResult) => {
        if (err) {
            ...
            dbConnection.query(`select * from comment where id_post = ${req.params.id}`, (err, rslt)=>{
                if (err) {
                    console.log(err)
                }
                else {
            ...
            dbConnection.query(`select * from category where id_category = ${post.id_category}`, (err, result) => {
                if (err) {
                    console.log(err)
                }
                else {
...
router.get('/search', (req, res) => {
    ...
    dbConnection.query(sql, (err, queryResut) => {
        if (err) {
            ...
            let matchedPosts = queryResut.filter(p => { if (p.title.includes(keyword)) return p })
